...
def main(
    ...
    data_path: dp.DataPath = dp.DataPath(program=code, io_controller=io_controller)
    control_unit: cu.ControlUnit = cu.ControlUnit(data_path=data_path)
    data_path.control_unit = control_unit

    ticks, instructions = control_unit.run_simulation(
        tick_limit=tick_limit, debug_limit=debug_limit
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-l", "--limit", type=int, default=200000, help="Tick limit (default - 200000)"
    )
    parser.add_argument(
        "-c", "--char", action="store_true", help="Char output (default - list[int])"
    )
    parser.add_argument(
        "-d", "--debug", action="store_true", help="Debug logging (default - info)"
    )
