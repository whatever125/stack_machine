...
- Стек данных реализован в виде отдельного регистра, представляющего вершину
  стека `TOS` и высокоуровневую структуру данных `deque`.
- Стек данных 32-разрядный и позволяет полностью помещать один операнд в одной
  ячейке памяти.
- Стек возврата реализован в виде отдельного регистра, представляющего 
  высокоуровневую структуры данных `deque`.
- Стек возврата 32-разрядный и позволяет полностью помещать адрес возврата
